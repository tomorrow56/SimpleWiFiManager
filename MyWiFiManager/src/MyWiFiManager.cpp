...
void MyWiFiManager::addParameter(WiFiManagerParameter *p) {
...
boolean MyWiFiManager::configPortalHasTimeout(){
...
boolean MyWiFiManager::startConfigPortal() {
  String ssid = "ESP" + String(ESP.getEfuseMac());
...
boolean MyWiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  _webUI->setupHandlers(
    std::bind(&MyWiFiManager::handleRoot, this),
    std::bind(&MyWiFiManager::handleWifi, this, std::placeholders::_1),
    std::bind(&MyWiFiManager::handleWifiSave, this),
    std::bind(&MyWiFiManager::handleInfo, this),
    std::bind(&MyWiFiManager::handleReset, this),
    std::bind(&MyWiFiManager::handleNotFound, this),
    std::bind(&MyWiFiManager::captivePortal, this),
    std::bind(&MyWiFiManager::handleThemeToggle, this)
  );
...
void MyWiFiManager::setWebUITheme(int theme) {
  if (_webUI) {
    _webUI->setTheme(theme);
  }
}

int MyWiFiManager::getWebUITheme() {
  if (_webUI) {
...
void MyWiFiManager::handleThemeToggle() {
...
void MyWiFiManager::setAPCallback(void (*func)(MyWiFiManager*)) {
  _apcallback = func;
}
...
void MyWiFiManager::setWebUITitle(const char* title) {
  _webUITitle = title;
}
